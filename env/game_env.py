...
class GameEnv:
    def __init__(self, screen_mode="low"):
        ...
        self.edge_guard_pen = 0.08
         ...
         # =========================================================
        # ✅ (추가) 절대 거리 페널티(매 프레임)
        # - 목표 y(ty)와 현재 y(py)의 "절대거리"를 항상 벌점으로 준다
        # - 정규화 버전: |py-ty| / h  (h=플레이필드 높이)
        # - 너무 세면 움직임이 줄어들 수 있으니 k는 낮게 시작
        # =========================================================
        self.abs_y_penalty_k = 0.06   # ← 추천 시작값 (0.03~0.12 사이 튜닝)
    ...
    def step(self, action_idx):
        ...
        for _ in range(self.s.action_repeat):
            ...
            if pc is not None:
                ...
                # =========================================================
                # ✅ (추가) 절대 거리 페널티: 매 프레임 항상 적용
                # - y만 쓰는 게 목적이라 |py-ty|만 사용
                # - 플레이필드 높이 h로 정규화해서 해상도 바뀌어도 안정
                # =========================================================
                abs_y_norm = float(abs(py - ty) / max(1.0, h))
                reward -= float(self.abs_y_penalty_k * abs_y_norm)
