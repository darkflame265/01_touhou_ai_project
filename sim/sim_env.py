# sim/sim_env.py
from __future__ import annotations

from dataclasses import dataclass
from typing import Dict, Tuple, Optional, List, Any

import numpy as np
import cv2

# ---- Action compatibility layer (prefer project actions) ----
try:
    from env.actions import ACTIONS as PROJECT_ACTIONS
except Exception:
    PROJECT_ACTIONS = None


# -----------------------------
# STOP action placeholder
# -----------------------------
@dataclass(frozen=True)
class _StopAction:
    name: str = "STOP"


# -----------------------------
# Config
# -----------------------------
@dataclass
class SimConfig:
    # obs 규약(네 ObsBuilder와 동일)
    obs_out_size: int = 128
    world_size: int = 256

    # player (base / default)
    player_speed: float = 4.0
    player_radius: float = 4.0

    # bullets pool
    max_bullets: int = 512
    bullet_radius: float = 2.0

    # termination
    hit_radius: float = 5.0
    max_steps: int = 3000

    # boss / emitter
    boss_y: float = 0.18
    boss_x_jitter: float = 0.12
    pattern_hold_steps: int = 240

    # difficulty knobs
    difficulty: int = 0  # 0..3 추천

    # reward
    alive_reward: float = 0.02
    hit_penalty: float = 1.0

    # move/energy penalty
    move_penalty: float = 0.001

    # near-miss shaping
    near_k: float = 0.08
    near_tau: float = 18.0

    # bottom wall hugging penalty
    bottom_wall_k: float = 0.02
    bottom_wall_margin: float = 28.0

    # background domain randomization
    bg_enable: bool = True
    bg_noise_amp: float = 18.0
    bg_flicker_amp: float = 22.0

    # seed
    seed: int = 0

    # render (debug)
    # ※ 실제 ON/OFF는 runner의 --no-render에서 cfg.render를 꺼주는 구조를 권장
    render: bool = True
    render_upscale: int = 2
    render_wait: int = 1
    render_window: str = "SIM"

    # -----------------------------
    # episode-wise speed randomization (sim2real robustness)
    # -----------------------------
    speed_randomize: bool = True

    # player speed range (absolute)
    player_speed_min: float = 3.0
    player_speed_max: float = 5.0

    # bullet speed scale range (multiplier on emitter's computed spd)
    bullet_speed_scale_min: float = 0.80
    bullet_speed_scale_max: float = 1.25

    # keep sampled values fixed for N episodes (1 = every episode)
    speed_resample_every_episodes: int = 1


# -----------------------------
# Bullet patterns (Touhou-like)
# -----------------------------
class PatternEmitter:
    def __init__(self, rng: np.random.Generator):
        self.rng = rng
        self.pattern_id = 0
        self.phase = 0.0
        self.hold_left = 0

    def reset(self, hold_steps: int):
        # ✅ 2패턴만
        self.pattern_id = int(self.rng.integers(0, 2))  # {0,1}
        self.phase = float(self.rng.uniform(0, 2 * np.pi))
        self.hold_left = int(hold_steps)

    def maybe_switch(self, hold_steps: int):
        self.hold_left -= 1
        if self.hold_left <= 0:
            self.reset(hold_steps)

    def spawn(self, env: "SimEnv"):
        cfg = env.cfg
        bx, by = env.boss_xy

        d = int(cfg.difficulty)
        base_speed = 2.6 + 0.6 * d
        base_n = 10 + 4 * d
        base_rate = 6 - min(3, d)

        t = env.t
        pid = int(self.pattern_id)

        if (t % max(1, base_rate)) != 0:
            return

        # episode-wise bullet speed scale
        spd_scale = float(getattr(env, "_bullet_speed_scale_ep", 1.0))

        if pid == 0:
            # radial burst
            n = base_n + int(self.rng.integers(0, 6))
            spd = base_speed * self.rng.uniform(0.9, 1.15) * spd_scale
            a0 = self.phase + 0.25 * np.sin(0.03 * t)
            for k in range(n):
                ang = a0 + 2 * np.pi * (k / n)
                env._spawn_bullet_xyv(bx, by, spd * np.cos(ang), spd * np.sin(ang))
            self.phase += 0.08
        else:
            # rotating ring
            n = base_n + 6
            spd = (1.9 + 0.35 * d) * self.rng.uniform(0.95, 1.15) * spd_scale
            a0 = self.phase + 0.12 * t
            for k in range(n):
                ang = a0 + 2 * np.pi * (k / n)
                env._spawn_bullet_xyv(bx, by, spd * np.cos(ang), spd * np.sin(ang))


# -----------------------------
# Env
# -----------------------------
class SimEnv:
    """
    OBS compatible (4ch float32):
      ch0: gray + marker + meta(x,y,conf)
      ch1: prev gray
      ch2: absdiff(current, prev)
      ch3: gaussian position hint
    """

    def __init__(self, cfg: Optional[SimConfig] = None):
        self.cfg = cfg or SimConfig()
        self.rng = np.random.default_rng(self.cfg.seed)

        # ✅ actions: 8-dir + STOP = 9
        self._actions = self._build_actions()

        # playfield
        self.s = int(self.cfg.obs_out_size)
        self.W = int(self.cfg.world_size)
        self.H = int(self.cfg.world_size)

        self._x0, self._x1 = 0, self.W
        self._y0, self._y1 = 0, self.H
        self._pw = self._x1 - self._x0
        self._ph = self._y1 - self._y0

        # meta/marker
        self.meta_patch = 4
        self.mark_player_on_ch0 = True
        self.marker_half = 2
        self.marker_value = 1.0
        self.marker_use_conf = True
        self.marker_min_scale = 0.35

        # hint
        self.player_hint_enable = True
        self.player_hint_sigma = 2.0
        self.player_hint_peak = 1.0
        self._grid_xy: Optional[Tuple[np.ndarray, np.ndarray]] = None

        # state
        self.t = 0
        self.episode = 0
        self.player_xy = np.zeros(2, dtype=np.float32)
        self.boss_xy = np.zeros(2, dtype=np.float32)

        # bullets pool
        self._cap = int(max(32, self.cfg.max_bullets))
        self._b_pos = np.zeros((self._cap, 2), dtype=np.float32)
        self._b_vel = np.zeros((self._cap, 2), dtype=np.float32)
        self._b_alive = np.zeros((self._cap,), dtype=np.bool_)
        self._b_n = 0

        # tracker-equivalent
        self.last_xy_norm: Tuple[float, float] = (0.5, 0.78)
        self.last_conf: float = 1.0
        self._uv = (self.s // 2, self.s // 2)

        # prev gray u8 (resized)
        self._prev_gray_u8: Optional[np.ndarray] = None

        # buffers
        self._z_u8 = np.zeros((self.s, self.s), np.uint8)
        self._obs = np.empty((4, self.s, self.s), np.float32)

        # ✅ A방법: world는 GRAY로 직접 그린다 (BGR 제거)
        self._world_gray = np.zeros((self.H, self.W), np.uint8)

        # emitter
        self.emitter = PatternEmitter(self.rng)

        # background (uint8 gray)
        self._bg0: Optional[np.ndarray] = None
        self._bg_phase = float(self.rng.uniform(0, 2 * np.pi))

        # episode-wise randomized speeds
        self._player_speed_ep = float(self.cfg.player_speed)
        self._bullet_speed_scale_ep = 1.0

    # ---------- public API ----------
    def reset(self, seed: Optional[int] = None) -> np.ndarray:
        if seed is not None:
            self.rng = np.random.default_rng(seed)
            self.emitter = PatternEmitter(self.rng)

        self.episode += 1
        self.t = 0
        self.player_xy[:] = (self.W * 0.5, self.H * 0.80)
        self._clear_bullets()

        # sample episode speeds
        self._maybe_resample_episode_speeds()

        self._set_boss_xy()
        self.emitter.reset(self.cfg.pattern_hold_steps)

        self.last_conf = 1.0
        self._update_last_xy_norm_and_uv()
        self._prev_gray_u8 = None

        self._init_bg()
        return self._build_obs()

    def step(self, action_idx: Any) -> Tuple[np.ndarray, float, bool, Dict]:
        self.t += 1

        # 1) player move
        dx, dy = self._action_to_delta(action_idx)
        moved = (dx != 0.0) or (dy != 0.0)

        self.player_xy[0] = np.clip(self.player_xy[0] + dx, 0, self.W - 1)
        self.player_xy[1] = np.clip(self.player_xy[1] + dy, 0, self.H - 1)

        # 2) boss drift
        self._set_boss_xy()

        # 3) spawn bullets
        self.emitter.spawn(self)
        self.emitter.maybe_switch(self.cfg.pattern_hold_steps)

        # 4) move bullets
        self._move_bullets()

        # 5) done / reward
        hit, dmin = self._check_hit_and_dmin()
        done = bool(hit or (self.t >= self.cfg.max_steps))

        reward = float(self.cfg.alive_reward)

        # move/energy penalty
        if moved:
            reward -= float(self.cfg.move_penalty)

        # hit / near-miss shaping
        if hit:
            reward -= float(self.cfg.hit_penalty)
        else:
            if np.isfinite(dmin):
                reward -= float(self.cfg.near_k) * float(np.exp(-float(dmin) / float(self.cfg.near_tau)))

        # bottom wall hugging penalty (quadratic)
        m = float(self.cfg.bottom_wall_margin)
        if m > 0.0:
            y = float(self.player_xy[1])
            dist_to_bottom = float((self.H - 1) - y)
            if dist_to_bottom < m:
                u = (m - dist_to_bottom) / m
                reward -= float(self.cfg.bottom_wall_k) * float(u * u)

        obs = self._build_obs()

        info = {
            "t": int(self.t),
            "episode": int(self.episode),
            "player_xy": self.player_xy.copy(),
            "boss_xy": self.boss_xy.copy(),
            "bullet_n": int(self._b_n),
            "hit": bool(hit),
            "dmin": float(dmin) if np.isfinite(dmin) else 1e9,
            "pattern": int(self.emitter.pattern_id),
            "player_speed_ep": float(self._player_speed_ep),
            "bullet_speed_scale_ep": float(self._bullet_speed_scale_ep),
        }
        return obs, float(reward), bool(done), info

    # ---------- episode speed randomization ----------
    def _maybe_resample_episode_speeds(self) -> None:
        cfg = self.cfg
        if not bool(cfg.speed_randomize):
            self._player_speed_ep = float(cfg.player_speed)
            self._bullet_speed_scale_ep = 1.0
            return

        every = int(max(1, cfg.speed_resample_every_episodes))
        if (self.episode % every) != 0:
            return

        # player speed (absolute)
        pmin = float(cfg.player_speed_min)
        pmax = float(cfg.player_speed_max)
        if pmax < pmin:
            pmin, pmax = pmax, pmin
        self._player_speed_ep = float(self.rng.uniform(pmin, pmax))

        # bullet speed scale (multiplier)
        bmin = float(cfg.bullet_speed_scale_min)
        bmax = float(cfg.bullet_speed_scale_max)
        if bmax < bmin:
            bmin, bmax = bmax, bmin
        self._bullet_speed_scale_ep = float(self.rng.uniform(bmin, bmax))

    # ---------- bullets pool ----------
    def _clear_bullets(self):
        self._b_alive[:] = False
        self._b_n = 0

    def _spawn_bullet_xyv(self, x: float, y: float, vx: float, vy: float):
        if self._b_n >= self._cap:
            return
        idx = self._b_n
        self._b_pos[idx, 0] = float(x)
        self._b_pos[idx, 1] = float(y)
        self._b_vel[idx, 0] = float(vx)
        self._b_vel[idx, 1] = float(vy)
        self._b_alive[idx] = True
        self._b_n += 1

    def _move_bullets(self):
        if self._b_n <= 0:
            return

        pos = self._b_pos[: self._b_n]
        vel = self._b_vel[: self._b_n]
        alive = self._b_alive[: self._b_n]

        pos[alive] += vel[alive]

        x = pos[:, 0]
        y = pos[:, 1]
        inb = (x >= -12) & (x <= self.W + 12) & (y >= -12) & (y <= self.H + 12)
        alive &= inb
        self._b_alive[: self._b_n] = alive

        if (self.t % 30) == 0:
            self._compact_bullets()

    def _compact_bullets(self):
        if self._b_n <= 0:
            self._b_n = 0
            return
        alive = self._b_alive[: self._b_n]
        if bool(alive.all()):
            return
        idxs = np.nonzero(alive)[0]
        newn = int(idxs.shape[0])
        if newn <= 0:
            self._clear_bullets()
            return
        self._b_pos[:newn] = self._b_pos[idxs]
        self._b_vel[:newn] = self._b_vel[idxs]
        self._b_alive[:newn] = True
        self._b_alive[newn : self._b_n] = False
        self._b_n = newn

    # ---------- obs ----------
    def _build_obs(self) -> np.ndarray:
        # world gray
        world_gray_u8 = self._render_world_gray_u8()

        # optional debug render (world)
        if self.cfg.render:
            vis = cv2.cvtColor(world_gray_u8, cv2.COLOR_GRAY2BGR)
            up = max(1, int(self.cfg.render_upscale))
            if up != 1:
                vis = cv2.resize(
                    vis,
                    (vis.shape[1] * up, vis.shape[0] * up),
                    interpolation=cv2.INTER_NEAREST,
                )

            txt = f"ep{self.episode} pspd={self._player_speed_ep:.2f} bmul={self._bullet_speed_scale_ep:.2f}"
            cv2.putText(vis, txt, (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2, cv2.LINE_AA)
            cv2.putText(vis, txt, (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 0), 1, cv2.LINE_AA)

            cv2.imshow(self.cfg.render_window, vis)
            cv2.waitKey(int(self.cfg.render_wait))

        pf = world_gray_u8[self._y0:self._y1, self._x0:self._x1]
        interp = cv2.INTER_AREA if max(pf.shape[:2]) >= self.s else cv2.INTER_LINEAR
        gray_u8 = cv2.resize(pf, (self.s, self.s), interpolation=interp)

        # prev/diff
        if self._prev_gray_u8 is None or self._prev_gray_u8.shape != gray_u8.shape:
            prev_u8 = self._z_u8
            diff_u8 = self._z_u8
        else:
            prev_u8 = self._prev_gray_u8
            diff_u8 = cv2.absdiff(gray_u8, prev_u8)

        hint = self._player_hint_map()

        # assemble (float32 0..1)
        self._obs[0] = gray_u8.astype(np.float32) * (1.0 / 255.0)
        self._stamp_marker(self._obs[0])
        self._inject_meta(self._obs[0])

        self._obs[1] = prev_u8.astype(np.float32) * (1.0 / 255.0)
        self._obs[2] = diff_u8.astype(np.float32) * (1.0 / 255.0)
        self._obs[3] = hint

        self._prev_gray_u8 = gray_u8
        return self._obs.copy()

    # ---------- background / render helpers ----------
    def _init_bg(self):
        if not self.cfg.bg_enable:
            self._bg0 = None
            return
        base = self.rng.normal(0.0, 1.0, size=(self.H, self.W)).astype(np.float32)
        base = cv2.GaussianBlur(base, (0, 0), sigmaX=6.0, sigmaY=6.0)
        base -= base.min()
        denom = float(base.max() - base.min() + 1e-6)
        base = base / denom
        self._bg0 = (base * float(self.cfg.bg_noise_amp)).astype(np.uint8)

    def _render_world_gray_u8(self) -> np.ndarray:
        img = self._world_gray
        img.fill(0)

        # background
        if self._bg0 is not None:
            flick = float(self.cfg.bg_flicker_amp) * (0.5 + 0.5 * np.sin(0.03 * self.t + self._bg_phase))
            bg = self._bg0.astype(np.float32) + flick
            np.clip(bg, 0, 255, out=bg)
            img[:] = bg.astype(np.uint8)

        # boss (faint)
        cv2.circle(img, (int(self.boss_xy[0]), int(self.boss_xy[1])), 5, 60, -1, lineType=cv2.LINE_AA)

        # bullets
        n = int(self._b_n)
        if n > 0:
            pos = self._b_pos[:n]
            alive = self._b_alive[:n]
            for x, y in pos[alive]:
                c = int(120 + 60 * self.rng.uniform(0.0, 1.0))
                cv2.circle(img, (int(x), int(y)), int(self.cfg.bullet_radius), c, -1, lineType=cv2.LINE_AA)

        # player
        cv2.circle(
            img,
            (int(self.player_xy[0]), int(self.player_xy[1])),
            int(self.cfg.player_radius),
            220,
            -1,
            lineType=cv2.LINE_AA,
        )
        return img

    # ---------- boss ----------
    def _set_boss_xy(self):
        bx0 = 0.5 + float(self.cfg.boss_x_jitter) * np.sin(0.01 * self.t + 0.7)
        by0 = float(self.cfg.boss_y)
        self.boss_xy[0] = float(np.clip(bx0, 0.1, 0.9) * (self.W - 1))
        self.boss_xy[1] = float(np.clip(by0, 0.05, 0.35) * (self.H - 1))

    # ---------- meta/marker ----------
    def _inject_meta(self, ch0: np.ndarray) -> None:
        p = int(self.meta_patch)
        if p <= 0 or ch0.shape[0] < p or ch0.shape[1] < p * 3:
            return
        x_n, y_n = self.last_xy_norm
        c = float(np.clip(self.last_conf, 0.0, 1.0))
        ch0[0:p, 0:p] = float(np.clip(x_n, 0.0, 1.0))
        ch0[0:p, p:2 * p] = float(np.clip(y_n, 0.0, 1.0))
        ch0[0:p, 2 * p:3 * p] = c

    def _stamp_marker(self, ch0: np.ndarray) -> None:
        if not self.mark_player_on_ch0:
            return
        u, v = self._uv
        r = int(self.marker_half)
        if r <= 0:
            return
        val = float(self.marker_value)
        if self.marker_use_conf:
            val *= max(float(self.marker_min_scale), float(np.clip(self.last_conf, 0.0, 1.0)))
        x1, x2 = max(0, u - r), min(self.s, u + r + 1)
        y1, y2 = max(0, v - r), min(self.s, v + r + 1)
        ch0[v, x1:x2] = val
        ch0[y1:y2, u] = val

    # ---------- hint map ----------
    def _get_grid(self):
        if self._grid_xy is not None:
            return self._grid_xy
        xs = np.arange(self.s, dtype=np.float32)
        ys = np.arange(self.s, dtype=np.float32)
        xx, yy = np.meshgrid(xs, ys)
        self._grid_xy = (xx, yy)
        return self._grid_xy

    def _player_hint_map(self) -> np.ndarray:
        if not self.player_hint_enable:
            return np.zeros((self.s, self.s), np.float32)

        sigma = float(max(1e-6, self.player_hint_sigma))
        peak = float(np.clip(self.player_hint_peak, 0.0, 1.0))

        xx, yy = self._get_grid()
        u, v = self._uv
        du = xx - float(u)
        dv = yy - float(v)
        d2 = du * du + dv * dv
        hint = np.exp(-d2 / (2.0 * sigma * sigma)).astype(np.float32)
        if peak != 1.0:
            hint *= peak
        return hint

    # ---------- xy norm + uv ----------
    def _xy_norm(self, cx: float, cy: float) -> Tuple[float, float]:
        x_pf = float(cx - self._x0)
        y_pf = float(cy - self._y0)
        x_n = float(np.clip(x_pf / max(1, self._pw - 1), 0.0, 1.0))
        y_n = float(np.clip(y_pf / max(1, self._ph - 1), 0.0, 1.0))
        return x_n, y_n

    def _update_uv(self) -> None:
        px, py = float(self.player_xy[0]), float(self.player_xy[1])
        x_pf = float(px - self._x0)
        y_pf = float(py - self._y0)
        u = int(round(x_pf * (self.s - 1) / max(1, (self._pw - 1))))
        v = int(round(y_pf * (self.s - 1) / max(1, (self._ph - 1))))
        self._uv = (int(np.clip(u, 0, self.s - 1)), int(np.clip(v, 0, self.s - 1)))

    def _update_last_xy_norm_and_uv(self) -> None:
        self.last_xy_norm = self._xy_norm(float(self.player_xy[0]), float(self.player_xy[1]))
        self._update_uv()

    # ---------- actions ----------
    def _build_actions(self) -> List[Any]:
        """
        ✅ 9 actions: 8-dir + STOP
        - 프로젝트 ACTIONS가 있어도 STOP은 우리가 추가(이름이 STOP으로 찍히게)
        """
        # base 8 moves
        if PROJECT_ACTIONS is None:
            moves8: List[Any] = list(range(8))
        else:
            want = [
                "SLOW_LEFT",
                "SLOW_RIGHT",
                "SLOW_UP",
                "SLOW_DOWN",
                "SLOW_UP_LEFT",
                "SLOW_UP_RIGHT",
                "SLOW_DOWN_LEFT",
                "SLOW_DOWN_RIGHT",
            ]
            by_name = {getattr(a, "name", ""): a for a in PROJECT_ACTIONS}
            picked = [by_name[n] for n in want if n in by_name]
            if len(picked) == 8:
                moves8 = picked
            else:
                tmp = []
                for a in PROJECT_ACTIONS:
                    name = getattr(a, "name", str(a)).upper()
                    if "BOMB" in name:
                        continue
                    tmp.append(a)
                moves8 = tmp[:8] if len(tmp) >= 8 else list(range(8))

        # append STOP as 9th
        return list(moves8) + [_StopAction()]

    def _action_to_delta(self, action_idx: Any) -> Tuple[float, float]:
        # episode-wise randomized player speed
        spd = float(getattr(self, "_player_speed_ep", self.cfg.player_speed))

        # accept: int index, or Action-like object
        if isinstance(action_idx, (int, np.integer)):
            i = int(action_idx) % len(self._actions)  # ✅ 9 actions
            act = self._actions[i]
        else:
            act = action_idx

        name = getattr(act, "name", "")
        n = name.upper()

        # ✅ STOP action
        if n == "STOP":
            self.last_conf = 1.0
            self._update_last_xy_norm_and_uv()
            return 0.0, 0.0

        dx, dy = 0.0, 0.0
        if "UP" in n:
            dy -= spd
        if "DOWN" in n:
            dy += spd
        if "LEFT" in n:
            dx -= spd
        if "RIGHT" in n:
            dx += spd

        # fallback for int-only (0..7)
        if name == "" and isinstance(act, (int, np.integer)):
            mapping = {
                0: (-spd, 0.0),
                1: ( spd, 0.0),
                2: (0.0, -spd),
                3: (0.0,  spd),
                4: (-spd, -spd),
                5: ( spd, -spd),
                6: (-spd,  spd),
                7: ( spd,  spd),
            }
            dx, dy = mapping[int(act) % 8]

        if dx != 0.0 and dy != 0.0:
            s = 1.0 / np.sqrt(2.0)
            dx *= s
            dy *= s

        self.last_conf = 1.0
        self._update_last_xy_norm_and_uv()
        return float(dx), float(dy)

    # ---------- hit check ----------
    def _check_hit_and_dmin(self) -> Tuple[bool, float]:
        n = int(self._b_n)
        if n <= 0:
            return False, float("inf")
        alive = self._b_alive[:n]
        if not bool(alive.any()):
            return False, float("inf")

        pos = self._b_pos[:n][alive]
        d = pos - self.player_xy[None, :]
        dist = np.sqrt(np.sum(d * d, axis=1))

        dmin = float(dist.min()) if dist.size else float("inf")
        hit = bool(np.any(dist <= float(self.cfg.hit_radius)))
        return hit, dmin
